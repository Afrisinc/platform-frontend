...
export const apiKeyService = {
  ...
  create: async (_wsId: string, data: { name: string; environment: ApiKey["environment"]; description?: string }): Promise<CreateApiKeyResponse> => {
    await delay();
    const prefix = data.environment === "Production" ? "ak_live_" : "ak_test_";
    const suffix = Math.random().toString(36).slice(2, 6);
    return {
      key: { id: crypto.randomUUID(), name: data.name, prefix: prefix + suffix, environment: data.environment, createdBy: "John Doe", createdAt: new Date().toISOString() },
