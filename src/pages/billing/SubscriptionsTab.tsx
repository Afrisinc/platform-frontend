    ...
    return (
      <div className="bg-card rounded-xl border border-border p-12 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          Activate a product from the Products page to get started.
        </p>
      </div>
    );
  }
...
function SubscriptionRow({
  ...
  const costLabel =
    sub.billingModel === "seat"
      ? `$${sub.pricePerSeat}/user × ${sub.seats}`
  ...
  return (
    <tr className="hover:bg-muted/20 transition-colors">
      ...
      <td className="px-5 py-4 capitalize">
        {sub.billingCycle} · {sub.billingModel}
      </td>
      <td className="px-5 py-4">
        <span
          className={`text-xs px-2.5 py-1 rounded-full font-medium capitalize ${sub.status === "active" ? "bg-success/10 text-success" : sub.status === "trialing" ? "bg-primary/10 text-primary" : "bg-destructive/10 text-destructive"}`}
        >
          {sub.status}
        </span>
      </td>
