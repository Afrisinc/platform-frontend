    ...
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {methods.map((pm) => {
            ...
            return (
              <div
                key={pm.id}
                className={`bg-card rounded-xl border p-5 transition-shadow hover:shadow-md ${pm.isDefault ? "border-primary" : "border-border"}`}
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div
                      className={`p-2.5 rounded-lg ${pm.isDefault ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"}`}
                    >
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={() => removePM.mutate(pm.id)}
                    className="flex items-center gap-1 text-xs font-medium text-destructive hover:underline ml-auto"
                  >
      ...
      <div className="bg-muted/50 rounded-lg border border-border p-4 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            We're working on adding MTN MoMo, Airtel Money, and more.
          </p>
        </div>
      </div>
