...
export default function UsageTab() {
  ...
  if (isLoading) {
    return <div className="space-y-6">{Array.from({ length: 2 }).map((_, i) => <Skeleton key={i} className="h-72 rounded-xl" />)}</div>;
  }
  ...
  if (!metrics?.length) {
    return (
      <div className="bg-card rounded-xl border border-border p-12 text-center">
        ...
        <p className="text-sm text-muted-foreground">Usage metrics will appear once metered products are active.</p>
      </div>
    );
  }
      ...
      {metrics.map((m) => (
        <div key={m.productId} className="bg-card rounded-xl border border-border p-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ backgroundColor: `hsl(${m.color} / 0.12)` }}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">
            ...
            {m.limit && <MiniStat label="Used" value={`${((m.currentValue / m.limit) * 100).toFixed(1)}%`} />}
          </div>
          ...
          {m.limit && (
            <div className="mb-6">
              <div className="flex justify-between text-xs text-muted-foreground mb-1">
                <span>{m.currentValue.toLocaleString()} / {m.limit.toLocaleString()} {m.unit}</span>
                <span>{((m.currentValue / m.limit) * 100).toFixed(1)}%</span>
              </div>
              <div className="h-2 rounded-full bg-muted overflow-hidden">
                <div
                  className="h-full rounded-full transition-all"
                  style={{ width: `${Math.min((m.currentValue / m.limit) * 100, 100)}%`, backgroundColor: `hsl(${m.color})` }}
                />
              </div>
            </div>
          )}
          ...
          <div className="h-48">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={m.dailyUsage} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                ...
                <XAxis dataKey="date" tick={{ fontSize: 10 }} tickFormatter={(v: string) => v.slice(-2)} className="text-muted-foreground" />
