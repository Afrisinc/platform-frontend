    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          value={`$${data.estimatedTotal.toFixed(2)}`}
        ...
        <StatCard
          ...
          value={`$${data.subscriptionCharges.toFixed(2)}`}
          sub={`${data.subscriptions.length} active products`}
          color="success"
        />
        <StatCard
          icon={TrendingUp}
          label="Usage Charges"
          value={`$${data.usageCharges.toFixed(2)}`}
          sub="metered billing"
          color="warning"
        />
        <StatCard
            ...
            <div key={sub.productId} className="flex items-center justify-between py-3">
              <div className="flex items-center gap-3">
                <div
                  ...
                  style={{ backgroundColor: `hsl(${sub.color} / 0.12)` }}
                >
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {sub.plan} ·{" "}
                    {sub.billingModel === "seat" ? `${sub.seats} seats` : sub.billingModel}
                  </p>
                </div>
              </div>
              <div className="text-right">
                ...
                <span
                  className={`text-xs px-2 py-0.5 rounded-full font-medium ${sub.status === "active" ? "bg-success/10 text-success" : sub.status === "trialing" ? "bg-primary/10 text-primary" : "bg-destructive/10 text-destructive"}`}
                >
                  {sub.status}
                </span>
              </div>
