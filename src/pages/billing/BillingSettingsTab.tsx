...
export default function BillingSettingsTab() {
  ...
  const set = (key: keyof BillingSettings, value: string | boolean) => setForm({ ...form, [key]: value });
  const setAddr = (key: keyof BillingSettings["address"], value: string) => setForm({ ...form, address: { ...form.address, [key]: value } });
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      ...
      <Section title="Billing Address">
        <Field label="Address line 1" value={form.address.line1} onChange={(v) => setAddr("line1", v)} />
        <Field label="Address line 2" value={form.address.line2} onChange={(v) => setAddr("line2", v)} />
        <div className="grid grid-cols-2 gap-4">
          <Field label="City" value={form.address.city} onChange={(v) => setAddr("city", v)} />
          <Field label="State / Province" value={form.address.state} onChange={(v) => setAddr("state", v)} />
        </div>
        <div className="grid grid-cols-2 gap-4">
          <Field label="Postal code" value={form.address.postalCode} onChange={(v) => setAddr("postalCode", v)} />
          <Field label="Country" value={form.address.country} onChange={(v) => setAddr("country", v)} />
        </div>
      </Section>
      ...
      <Section title="Preferences">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Automatically charge default payment method on invoice date.</p>
          </div>
          <button
          ...
          >
            <span className={`absolute top-0.5 w-5 h-5 rounded-full bg-card shadow transition-transform ${form.autoPay ? "left-[22px]" : "left-0.5"}`} />
          </button>
        </div>
